...
app.post('/welfare', (req, res) => {
  ...
  const rows = []
  for (let i = 0; i < req.body.length; i++) {
    rows.push([req.body[i].TRIP_NO, req.body[i].TRIP_ALLOWANCE, req.body[i].TOTAL_ALLOWANCE, req.body[i].OT_HOURS, req.body[i].DEPARTURE_POINT, new Date((req.body[i].DEPARTURE_DATETIME - 1) * 24 * 60 * 60 * 1000 + new Date(1900, 0, 0).getTime()), new Date((req.body[i].YARDOUTDATE - 1) * 24 * 60 * 60 * 1000 + new Date(1900, 0, 0).getTime()), req.body[i].DRIVER1, req.body[i].NAME, req.body[i].DRIVER2, req.body[i].DEALER1, req.body[i].DEALER2, req.body[i].DEALER3, req.body[i].DEALER4, req.body[i].DEALER5, req.body[i].UNITS1, req.body[i].UNITS2, req.body[i].UNITS3, req.body[i].UNITS4, req.body[i].UNITS5, req.body[i].TAX_FLAG])
    // rows.push([req.body[i].TRIP_NO, req.body[i].TRIP_ALLOWANCE, req.body[i].TOTAL_ALLOWANCE, req.body[i].OT_HOURS, req.body[i].DEPARTURE_POINT, req.body[i].DEPARTURE_DATETIME, req.body[i].YARDOUTDATE, req.body[i].DRIVER1, req.body[i].NAME, req.body[i].DRIVER2, req.body[i].NULLS, req.body[i].DEALER1, req.body[i].DEALER2, req.body[i].DEALER3, req.body[i].DEALER4, req.body[i].DEALER5, req.body[i].UNITS1, req.body[i].UNITS2, req.body[i].UNITS3, req.body[i].UNITS4, req.body[i].UNITS5, req.body[i].TAX_FLAG])
    // rows.push([`emp_code${i}`, `name${i}`, `bank_account_number${i}`])
  }
  // console.log('data', rows)
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    console.log("Connected!")
    var sql = 'INSERT INTO welfare (TRIP_NO, TRIP_ALLOWANCE, TOTAL_ALLOWANCE, OT_HOURS, DEPARTURE_POINT, DEPARTURE_DATETIME, YARDOUTDATE, DRIVER1, NAME, DRIVER2, DEALER1, DEALER2, DEALER3, DEALER4, DEALER5, UNITS1, UNITS2, UNITS3, UNITS4, UNITS5, TAX_FLAG) VALUES ?';
    var value = [req.body.emp_code, req.body.name, req.body.bank_account_number];
    // console.log('dataSQL', sql)
    // console.log('dataROWs', [rows])
    if (err) throw err
...
app.post('/instructor', (req, res) => {
  ...
  const rows = []
  for (let i = 0; i < req.body.length; i++) {
    rows.push([req.body[i].number, req.body[i].TRIP_NO, req.body[i].TRIP_ALLOWANCE, req.body[i].TOTAL_ALLOWANCE, new Date((req.body[i].DEPARTURE_DATETIME - 1) * 24 * 60 * 60 * 1000 + new Date(1900, 0, 0).getTime()), new Date((req.body[i].YARDOUTDATE) * 24 * 60 * 60 * 1000 + new Date(1900, 0, 0).getTime()), req.body[i].DRIVER1, req.body[i].NAME, req.body[i].DEALER1, req.body[i].DEALER2, req.body[i].DEALER3, req.body[i].DEALER4, req.body[i].DEALER5, req.body[i].UNITS1, req.body[i].UNITS2, req.body[i].UNITS3, req.body[i].UNITS4, req.body[i].UNITS5, req.body[i].TAX_FLAG])
    // rows.push([`emp_code${i}`, `name${i}`, `bank_account_number${i}`])
  }
  // console.log('data', rows)
  // console.log('DATA: ', req.body.pallet)
  connection.getConnection((err, con) => {
    if (err) throw err
    console.log("Connected!")
    var sql = 'INSERT INTO instructor_controller (number, TRIP_NO, TRIP_ALLOWANCE, TOTAL_ALLOWANCE, DEPARTURE_DATETIME, YARDOUTDATE, DRIVER1, NAME, DEALER1, DEALER2, DEALER3, DEALER4, DEALER5, UNITS1, UNITS2, UNITS3, UNITS4, UNITS5, TAX_FLAG) VALUES ?';
    var value = [req.body.emp_code, req.body.name, req.body.bank_account_number];
    // console.log('dataSQL', sql)
    // console.log('dataROWs', [rows])
    if (err) throw err
...
app.post('/getdataattach7', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT employee.bank_account_number, employee.emp_code, employee.name ,sum(tnos_system5.total_allowance) AS total_allowance FROM tnos_system5 INNER JOIN employee on tnos_system5.ttt_employee_code = employee.emp_code WHERE DATE(tnos_system5.Working_date) BETWEEN ? AND ? AND tnos_system5.payment_status_2 is NULL GROUP BY tnos_system5.ttt_employee_code;"
    var value = [req.body.from, req.body.to];
    // connection.query("SELECT employee.bank_account_number, employee.emp_code, employee.name ,sum(tnos_system5.total_allowance) AS total_allowance FROM tnos_system5 INNER JOIN employee on tnos_system5.ttt_employee_code = employee.emp_code AND DATE(tnos_system5.Working_date) BETWEEN ? AND ? GROUP BY tnos_system5.ttt_employee_code;", (err, result, fields) => {
    if (err) throw err
    connection.query(sql, value, (err, result, fields) => {
...
app.post('/getdataattach72', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT employee.bank_account_number, employee.emp_code, employee.name ,welfare.total_allowance FROM welfare INNER JOIN employee on welfare.DRIVER1 = employee.emp_code WHERE DATE(DEPARTURE_DATETIME) BETWEEN ? AND ? AND welfare.payment_status_2 is NULL GROUP BY welfare.DRIVER1;"
    var value = [req.body.from, req.body.to];
...
app.post('/getdataattach73', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT employee.bank_account_number, employee.emp_code, employee.name ,instructor_controller.total_allowance FROM instructor_controller INNER JOIN employee on instructor_controller.DRIVER1 = employee.emp_code WHERE DATE(DEPARTURE_DATETIME) BETWEEN ? AND ? AND instructor_controller.payment_status_2 is NULL GROUP BY instructor_controller.DRIVER1;"
    var value = [req.body.from, req.body.to];
...
app.post('/getdataattach72P', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT employee.bank_account_number, employee.emp_code, employee.name ,welfare.total_allowance, welfare.payment_status_2 FROM welfare INNER JOIN employee on welfare.DRIVER1 = employee.emp_code WHERE DATE(DEPARTURE_DATETIME) BETWEEN ? AND ? GROUP BY welfare.DRIVER1;"
    var value = [req.body.from, req.body.to];
    // connection.query("SELECT employee.bank_account_number, employee.emp_code, employee.name ,welfare.total_allowance FROM welfare INNER JOIN employee on welfare.DRIVER1 = employee.emp_code GROUP BY welfare.DRIVER1;", (err, result, fields) => {
    if (err) throw err
    connection.query(sql, value, (err, result, fields) => {
...
app.post('/getdataattach73P', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT employee.bank_account_number, employee.emp_code, employee.name ,instructor_controller.total_allowance, instructor_controller.payment_status_2 FROM instructor_controller INNER JOIN employee on instructor_controller.DRIVER1 = employee.emp_code WHERE DATE(DEPARTURE_DATETIME) BETWEEN ? AND ? GROUP BY instructor_controller.DRIVER1;"
...
app.post('/getdataattach9', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT tnos_system5.ttt_employee_code ,tnos_system5.tlep_driver_name, sum(tnos_system5.total_ot) as total_ot FROM tnos_system5 WHERE DATE(tnos_system5.Working_date) BETWEEN ? AND ? AND total_ot != 0 AND total_ot IS NOT NULL GROUP BY tnos_system5.ttt_employee_code;"
    var value = [req.body.from, req.body.to];
    // connection.query("SELECT tnos_system5.ttt_employee_code ,tnos_system5.tlep_driver_name, sum(tnos_system5.total_ot) as total_ot FROM tnos_system5 GROUP BY tnos_system5.ttt_employee_code;", (err, result, fields) => {
    if (err) throw err
    connection.query(sql, value, (err, result, fields) => {
...
app.post('/getdataattach92', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT welfare.DRIVER1, welfare.NAME ,welfare.OT_HOURS FROM welfare WHERE DATE(DEPARTURE_DATETIME) BETWEEN ? AND ? AND OT_HOURS != 0 AND OT_HOURS IS NOT NULL GROUP BY welfare.DRIVER1;"
    var value = [req.body.from, req.body.to];
...
app.post('/getdataattach93', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT instructor_controller.DRIVER1, instructor_controller.NAME ,instructor_controller.NULL1 FROM instructor_controller WHERE DATE(DEPARTURE_DATETIME) BETWEEN ? AND ? AND NULL1 != 0 AND NULL1 IS NOT NULL GROUP BY instructor_controller.DRIVER1;"
    var value = [req.body.from, req.body.to];
...
app.post('/getdataattach10', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT tnos_system5.calling_sheet_no ,tnos_system5.company_name, tnos_system5.standard_ot,total_allowance, tnos_system5.ttt_employee_code FROM tnos_system5 WHERE DATE(tnos_system5.Working_date) BETWEEN ? AND ? ORDER BY tnos_system5.ttt_employee_code;"
    var value = [req.body.from, req.body.to];
    // connection.query("SELECT tnos_system5.calling_sheet_no ,tnos_system5.company_name, tnos_system5.standard_ot,total_allowance, tnos_system5.ttt_employee_code FROM tnos_system5 WHERE DATE(tnos_system5.Working_date) BETWEEN ? AND ? ORDER BY tnos_system5.ttt_employee_code;", (err, result, fields) => {
    if (err) throw err
    connection.query(sql, value, (err, result, fields) => {
...
app.post('/getdatapayrollot', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT ttt_employee_code as EMP_CODE, sum(total_ot) as OT FROM tnos_system5 WHERE DATE(tnos_system5.Working_date) BETWEEN ? AND ? AND total_ot IS NOT NULL AND total_ot != 0 GROUP BY ttt_employee_code;"
    var value = [req.body.from, req.body.to];
    if (err) throw err
    connection.query(sql, value, (err, result, fields) => {
      if (err) {
...
app.post('/getdatapayrollallowance', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT ttt_employee_code as EMP_CODE, sum(total_allowance) as ALLOWANCE FROM tnos_system5 WHERE DATE(tnos_system5.Working_date) BETWEEN ? AND ? AND total_allowance IS NOT NULL AND total_allowance != 0 GROUP BY ttt_employee_code;"
    var value = [req.body.from, req.body.to];
...
app.post('/addpaymentstatusattach72', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    for (let i = 0; i < req.body.length; i++) {
      ...
      // rows.push([`emp_code${i}`, `name${i}`, `bank_account_number${i}`])
      if (err) throw err
      var sql = `UPDATE welfare SET payment_status_2 = (case when DRIVER1 = ${req.body[i].emp_code} then '${req.body[i].payment_status}' END), payment_date_st = (case when DRIVER1 = ${req.body[i].emp_code} then '${req.body[i].payment_date}' END) WHERE DRIVER1 in (${req.body[i].emp_code});`;
      var value = [req.body.payment_status, req.body.emp_code];
      // connection.query("SELECT DRIVER1 as EMP_CODE, sum(total_allowance) as ALLOWANCE FROM instructor_controller GROUP BY DRIVER1;", (err, result, fields) => {
      if (err) throw err
      connection.query(sql, (err, result, fields) => {
...
app.post('/addpaymentstatusattach73', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    for (let i = 0; i < req.body.length; i++) {
      ...
      // rows.push([`emp_code${i}`, `name${i}`, `bank_account_number${i}`])
      if (err) throw err
      var sql = `UPDATE instructor_controller SET payment_status_2 = (case when DRIVER1 = ${req.body[i].emp_code} then '${req.body[i].payment_status}' END), payment_date_st = (case when DRIVER1 = ${req.body[i].emp_code} then '${req.body[i].payment_date}' END) WHERE DRIVER1 in (${req.body[i].emp_code});`;
      var value = [req.body.payment_status, req.body.emp_code];
      // connection.query("SELECT DRIVER1 as EMP_CODE, sum(total_allowance) as ALLOWANCE FROM instructor_controller GROUP BY DRIVER1;", (err, result, fields) => {
      if (err) throw err
      connection.query(sql, (err, result, fields) => {
