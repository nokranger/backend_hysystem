...
app.post('/welfaregetdata', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT TRIP_NO, DRIVER1, NAME, DEPARTURE_DATETIME, YARDOUTDATE, DEALER1, TRIP_ALLOWANCE, OT_HOURS, UNITS1, TAX_FLAG, payment_status_2, payment_date_st from welfare WHERE DATE(DEPARTURE_DATETIME) BETWEEN ? AND ? GROUP BY DRIVER1;"
    var value = [req.body.from, req.body.to];
...
app.post('/instructorgetdata', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT number, TRIP_NO, DRIVER1, NAME, DEPARTURE_DATETIME, YARDOUTDATE, DEALER1, TRIP_ALLOWANCE, UNITS1, TAX_FLAG from instructor_controller WHERE DATE(DEPARTURE_DATETIME) BETWEEN ? AND ? GROUP BY DRIVER1;"
    var value = [req.body.from, req.body.to];
...
app.post('/getdataattach72', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT employee.bank_account_number, employee.emp_code, employee.name ,sum(welfare.total_allowance) AS total_allowance FROM welfare INNER JOIN employee on welfare.DRIVER1 = employee.emp_code WHERE DATE(DEPARTURE_DATETIME) BETWEEN ? AND ? AND welfare.payment_status_2 is NULL GROUP BY welfare.DRIVER1;"
    var value = [req.body.from, req.body.to];
...
app.post('/getdataattach9', (req, res) => {
  ...
  connection.getConnection((err, con) => {
    if (err) throw err
    var sql = "SELECT tnos_system5.ttt_employee_code ,tnos_system5.tlep_driver_name, sum(tnos_system5.total_ot) as total_ot FROM tnos_system5 WHERE DATE(recieve_job_dateandtime) BETWEEN ? AND ? AND total_ot != 0 AND total_ot IS NOT NULL GROUP BY tnos_system5.ttt_employee_code;"
    var value = [req.body.from, req.body.to];
    // connection.query("SELECT tnos_system5.ttt_employee_code ,tnos_system5.tlep_driver_name, sum(tnos_system5.total_ot) as total_ot FROM tnos_system5 GROUP BY tnos_system5.ttt_employee_code;", (err, result, fields) => {
    if (err) throw err
    connection.query(sql, value, (err, result, fields) => {
